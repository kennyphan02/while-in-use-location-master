...
class MainActivity : AppCompatActivity(), SharedPreferences.OnSharedPreferenceChangeListener, OnMapReadyCallback {
    ...
    override fun onMapReady(googleMap: GoogleMap) {
        ...
        val markerOptions = latLng?.let { MarkerOptions().position(it).title("Current Location") }
        latLng?.let { CameraUpdateFactory.newLatLng(it) }?.let { googleMap?.animateCamera(it) }
        latLng?.let { CameraUpdateFactory.newLatLngZoom(it, 7f ) }
